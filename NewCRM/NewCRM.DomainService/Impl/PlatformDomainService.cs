...
namespace NewCRM.DomainService.Impl
{
    public sealed class PlatformDomainService : IPlatformDomainService
    {
        ...
        private readonly IRepository<DeskMember> _deskMembeRepository;

        private readonly IRepository<UserConfigure> _userConfigureRepository;

        private readonly IRepository<Wallpaper> _wallpapeRepository;

        public PlatformDomainService()
        {
            _userRepository = RepositoryFactory<User>.CreateRepository();
            _deskMembeRepository = RepositoryFactory<DeskMember>.CreateRepository();
            _userConfigureRepository = RepositoryFactory<UserConfigure>.CreateRepository();
            _wallpapeRepository = RepositoryFactory<Wallpaper>.CreateRepository();
        }
        ...
        public IDictionary<Int32, Tuple<String, String>> GetWallpapers()
        {
            ...
            var wallpaperResult = _wallpapeRepository.Entities.ToList();
            foreach (var wallpaper in wallpaperResult)
            {
                wallpaperDictionary.Add(new KeyValuePair<int, Tuple<String, String>>(wallpaper.Id, new Tuple<String, String>(wallpaper.Url, wallpaper.Title)));
            }
