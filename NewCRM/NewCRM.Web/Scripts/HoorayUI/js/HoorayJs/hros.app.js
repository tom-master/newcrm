		...
		*/
        updateXY: function (i) {
            if (HROS.CONFIG.appXY !== i) {
                if (HROS.base.checkLogin()) {
                    $.post('/PlatformSetting/ChangeAppXy', { appXy: i }, function (result) {
                        if (parseInt(result.data) === 1) {
                            HROS.CONFIG.appXY = i;
                            HROS.deskTop.resize();
                            ZENG.msgbox.show('图标排列方向更新成功', 4, 2000);
                        } else {
                            ZENG.msgbox.show('图标排列方向更新失败', 5, 2000);
                        }
                    });
                }
            }
